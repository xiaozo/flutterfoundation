...
class _GradientAppBarState extends State<GradientAppBar> {
  ...
  Widget build(BuildContext context) {
    ...
    if (leading == null && widget.automaticallyImplyLeading && widget.leadings == null) {
      if (hasDrawer) {
        leading = IconButton(
          icon: const Icon(Icons.menu),
    ...
    Widget leadings;
    if (widget.leadings != null && widget.leadings.isNotEmpty && widget.leading == null) {
      if (widget.leadings.length > 1) {
        ...
        while (index < widget.leadings.length - 1) {
          ...
          if (!(tempWidget is IconButton))  {
            widget.leadings.insert(insertIndex, SizedBox(width: FoundationConfigUntil.navItemSpacing));
          }
          insertIndex+=2;
          index++;
        }
    ...
    if (widget.actions != null && widget.actions.isNotEmpty) {
      if (widget.actions.length > 1) {
        ...
        while (index < widget.actions.length - 1) {
          ...
          if (!(tempWidget is IconButton))  {
            widget.actions.insert(insertIndex, SizedBox(width: FoundationConfigUntil.navItemSpacing));
          }
          insertIndex+=2;
          index++;
        }
